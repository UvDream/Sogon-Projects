<template>
  <div>
    <p style="background-color:#d5d5d5;margin:0;padding:5px;">
      您当前处于
      <span class="tip" @mouseout="hoverFunc($event)">用户提交资料</span> 步骤
      下一步等待
      <span class="tip">供应商接单</span>
      <button type="text" v-if="show===false" @click="show=true">展开</button>
      <button type="text" v-else @click="show=false">收起</button>
    </p>
    <div id="myDiagramDiv" v-show="show" style="width:400px;height:400px"></div>
  </div>
</template>
<style scoped>
.tip {
  ...
  methods: {
    ...
    init() {
      ...
      this.myDiagram.nodeTemplate = $(
        ...
        $(go.Shape, "Rectangle", {
          fill: this.getOption()["yellowgrad"],
          stroke: "black",
          portId: "",
          fromLinkable: true,
          toLinkable: true,
        ...
        $(
          go.TextBlock,
          "Page",
          { margin: 6, font: this.getOption()["bigfont"], editable: true },
