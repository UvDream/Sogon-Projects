...
<template>
  <div class="dashboard-bottom" style="height:auto">
    <div class="dashboard-bottom-left">
      <Title title="最近半年电信网络受骗人员社保所属单位TOP5" v-model="data"/>
      <div class="dashboard-bottom-left-table">
        <div class="dashboard-bottom-left-table-left">
          <div>单位名称</div>
          <div v-for="(item,index) in tableList"  :key="index">{{item.dwname}}</div>
        </div>
  ...
  methods: {
    searchFunc(data) {
      ...
      api.fetchTablePaiming(data).then(res=>{
        ...
        if(res.data.lastHalfYearNetworkLiedTopFive.length > 0) {  
